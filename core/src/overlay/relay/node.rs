...
impl RelayNode {
                ...
                while let Some(msg) = rx.recv().await {
                    ...
                    match msg {
                        RelayMessage::AddStream(stream_id, publisher) => {
                            let _ =
                                Self::add_stream_internal(&streams, stream_id, publisher, &stats)
                                    .await;
                        }
                        RelayMessage::RemoveStream(stream_id) => {
                            let _ =
                                Self::remove_stream_internal(&streams, &stream_id, &stats).await;
                        }
                        RelayMessage::AddSubscriber(stream_id, peer_id) => {
                            let _ =
                                Self::add_subscriber_internal(&streams, stream_id, peer_id, &stats)
                                    .await;
                        }
                        RelayMessage::RemoveSubscriber(stream_id, peer_id) => {
                            let _ = Self::remove_subscriber_internal(
                                &streams, stream_id, peer_id, &stats,
                            )
                            .await;
                        }
    ...
    pub async fn publish_chunk(&self, chunk: StreamChunk) -> Result<(), OverlayError> {
        ...
        if chunk.data.len() > self.config.max_chunk_size {
            return Err(OverlayError::RelayError(format!(
    ...
    pub async fn add_stream(
        ...
        if let Some(tx) = self.message_tx.lock().await.as_ref() {
            if let Err(e) = tx
                .send(RelayMessage::AddStream(stream_id.clone(), publisher))
                .await
            {
                error!("Failed to send add stream message: {}", e);
                return Err(OverlayError::RelayError(format!(
                    "Failed to send add stream message: {}",
                    e
                )));
            }
            Ok(())
    ...
    pub async fn remove_stream(&self, stream_id: &StreamId) -> Result<(), OverlayError> {
        if let Some(tx) = self.message_tx.lock().await.as_ref() {
            if let Err(e) = tx.send(RelayMessage::RemoveStream(stream_id.clone())).await {
                ...
                return Err(OverlayError::RelayError(format!(
                    "Failed to send remove stream message: {}",
                    e
                )));
            }
            Ok(())
    ...
    pub async fn add_subscriber(
        ...
        if let Some(tx) = self.message_tx.lock().await.as_ref() {
            if let Err(e) = tx
                .send(RelayMessage::AddSubscriber(stream_id.clone(), peer_id))
                .await
            {
                error!("Failed to send add subscriber message: {}", e);
                return Err(OverlayError::RelayError(format!(
                    "Failed to send add subscriber message: {}",
                    e
                )));
            }
            Ok(())
    ...
    pub async fn remove_subscriber(
        ...
        if let Some(tx) = self.message_tx.lock().await.as_ref() {
                ...
                .await
            {
                ...
                return Err(OverlayError::RelayError(format!(
                    "Failed to send remove subscriber message: {}",
                    e
                )));
            }
            Ok(())
    ...
    pub async fn request_chunks(
        ...
        if let Some(tx) = self.message_tx.lock().await.as_ref() {
                ...
                .await
            {
                ...
                return Err(OverlayError::RelayError(format!(
                    "Failed to send request chunks message: {}",
                    e
                )));
            }
            Ok(())
