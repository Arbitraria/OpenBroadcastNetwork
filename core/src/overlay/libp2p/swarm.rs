...
impl Libp2pOverlay {
    ...
    pub async fn subscribe_to_stream(&self, stream_id: &StreamId) -> Result<(), OverlayError> {
        ...
        if let Err(e) = swarm.behaviour_mut().gossipsub.subscribe(&data_topic) {
            return Err(OverlayError::Other(format!("Failed to subscribe to data topic: {}", e)));
        }
        
        if let Err(e) = swarm.behaviour_mut().gossipsub.subscribe(&control_topic) {
            return Err(OverlayError::Other(format!("Failed to subscribe to control topic: {}", e)));
        }
    ...
    pub async fn unsubscribe_from_stream(&self, stream_id: &StreamId) -> Result<(), OverlayError> {
        ...
        if let Err(e) = swarm.behaviour_mut().gossipsub.unsubscribe(&data_topic) {
            return Err(OverlayError::Other(format!("Failed to unsubscribe from data topic: {}", e)));
        }
        
        if let Err(e) = swarm.behaviour_mut().gossipsub.unsubscribe(&control_topic) {
            return Err(OverlayError::Other(format!("Failed to unsubscribe from control topic: {}", e)));
        }
    ...
    pub async fn publish_message(&self, stream_id: &StreamId, data: Vec<u8>, is_control: bool) -> Result<(), OverlayError> {
        // Lock the swarm
        let mut swarm_lock = self.swarm.lock().await;
        ...
        match swarm.behaviour_mut().gossipsub.publish(topic, data) {
            Ok(_message_id) => {
                debug!("Published {} message to stream {}", 
                       if is_control { "control" } else { "data" },
                       stream_id);
                Ok(())
            },
            Err(e) => {
                Err(OverlayError::Other(format!("Failed to publish message: {}", e)))
            }
        }
    }
    ...
    pub async fn connect_peer(&self, peer_id: &PeerId) -> Result<(), OverlayError> {
        ...
        if !has_addresses {
            ...
            Err(OverlayError::Other(format!("Failed to connect to peer {}", peer_id)))
        }
    }
    ...
    pub async fn disconnect_peer(&self, peer_id: &PeerId) -> Result<(), OverlayError> {
        ...
        if let Some(connection_id) = swarm.connected_peers().find(|p| **p == *peer_id).map(|_| *peer_id) {
            // In newer versions, we would call swarm.close_connection or similar
            info!("Would disconnect from peer {} if API was available", peer_id);
        }
    ...
    pub async fn listen_on(&self, addr_str: &str) -> Result<(), OverlayError> {
        ...
        let addr = match addr_str.parse() {
            Ok(addr) => addr,
            Err(e) => return Err(OverlayError::Other(format!("Invalid address {}: {}", addr_str, e))),
        };
        ...
        match swarm.listen_on(addr) {
            ...
            Err(e) => Err(OverlayError::Other(format!("Failed to listen on {}: {}", addr_str, e))),
        }
    }
}
