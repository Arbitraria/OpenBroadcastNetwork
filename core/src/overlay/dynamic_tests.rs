...
mod dynamic_tests {
    ...
    use crate::overlay::peer::{Peer, PeerRole, PeerInfo};
    use crate::overlay::interface::StreamId;
    // Now that we've implemented TopologyManager, let's use it in our tests
    use crate::overlay::topology::{TopologyManager, TopologyConfig, ConnectionHealth};
    use libp2p::PeerId;
    ...
    async fn test_tree_rebalancing() {
        ...
        tree.nodes.get_mut(&relay1).unwrap().update_metrics(8_000_000, 50, 1.0);
        tree.nodes.get_mut(&relay2).unwrap().update_metrics(2_000_000, 100, 1.0);
        tree.nodes.get_mut(&relay3).unwrap().update_metrics(5_000_000, 75, 1.0);
        ...
        let initial_distribution = tree.nodes.iter()
        ...
        let rebalanced_distribution = tree.nodes.iter()
            .filter(|(_, node)| node.role == PeerRole::Relay)
            .map(|(id, node)| (*id, node.children.len()))
            .collect::<Vec<_>>();
