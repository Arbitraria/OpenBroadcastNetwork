                    ...
                    match swarm_instance.select_next_some().await {
                        ...
                        SwarmEvent::ConnectionEstablished {
                            ...
                            let peer_info = PeerInfo {
                                ...
                                connection_status:
                                    crate::discovery::interface::ConnectionStatus::Connected,
                            };
                            ...
                            // Store the peer
                            {
                                ...
                                peers_lock.insert(
                                    peer_id.to_bytes(),
                                    (peer_info.clone(), Instant::now()),
                                );
                            }

                            // Send event
                            let _ = event_sender
                                .send(DiscoveryEvent::PeerDiscovered(peer_info))
                                .await;
                        }
                        SwarmEvent::ConnectionClosed { peer_id, .. } => {
                            debug!("Disconnected from peer: {}", peer_id);
                            let _ = event_sender
                                .send(DiscoveryEvent::PeerExpired(peer_id.to_bytes()))
                                .await;
                        }
    ...
    async fn handle_kademlia_event(
        ...
        match event {
                    ...
                    QueryResult::GetClosestPeers(Ok(ok)) => {
                        ...
                        for peer_id in ok.peers {
                            let peer_info = PeerInfo {
                                ...
                                last_seen: Some(std::time::SystemTime::now()),
                                connection_status:
                                    crate::discovery::interface::ConnectionStatus::Disconnected,
                            };
            ...
            KademliaEvent::RoutingUpdated {
                ...
                // Send event
                let _ = event_sender
                    .send(DiscoveryEvent::PeerUpdated(peer_info))
                    .await;
            }
            _ => {}
        }
        ...
        // If we have a swarm, start providing this peer's record in the DHT
        {
            ...
            if let Some(ref mut swarm) = *swarm_lock {
                ...
                let query_id = swarm
                    ...
                    .map_err(|e| {
                        DiscoveryError::Other(format!("Failed to start providing: {}", e))
                    })?;
