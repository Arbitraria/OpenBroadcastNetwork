...
impl Discovery for BootstrapDiscovery {
    async fn start(&mut self) -> Result<(), DiscoveryError> {
        ...
        debug!(
            "Bootstrap discovery service started with peer ID: {:?}",
            self.local_peer_id
        );
        Ok(())
    }
    ...
    async fn announce(&mut self, info: PeerInfo) -> Result<(), DiscoveryError> {
        ...
        let mut peers = self
            .peers
            .lock()
            .map_err(|_| DiscoveryError::Other("Failed to lock peers".into()))?;
    ...
    async fn next_event(
        ...
        if let Some(receiver) = &mut self.event_receiver {
            match timeout {
                Some(duration) => match tokio::time::timeout(duration, receiver.next()).await {
