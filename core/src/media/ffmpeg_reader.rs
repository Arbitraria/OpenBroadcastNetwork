...
impl FFmpegVideoReader {
    ...
    fn analyze_streams(&mut self) -> Result<()> {
        ...
        // Find video stream
        if let Some(video_stream) = input
            .streams()
            .best(ffmpeg::media::Type::Video)
        {
            ...
            info!("Found video stream: {}x{} @ {:.2}fps, codec: {:?}", 
        ...
        // Find audio stream
        if let Some(audio_stream) = input
            .streams()
            .best(ffmpeg::media::Type::Audio)
        {
            ...
            let duration = Duration::from_secs_f64(
                audio_stream.duration() as f64 * time_base.numerator() as f64 / time_base.denominator() as f64
            );
            ...
            info!("Found audio stream: {} Hz, {} channels, codec: {:?}",
    ...
    pub fn start_streaming_conversion(&self, output_dir: &str) -> Result<std::process::Child> {
        ...
        let child = Command::new("ffmpeg")
            ...
            .arg("-segment_time").arg("2") // 2 second segments
            .arg("-segment_list_type").arg("m3u8")
            .arg("-segment_list").arg(format!("{}/playlist.m3u8", output_dir))
            .arg("-c:v").arg("libx264")
            .arg("-c:a").arg("aac")
            .arg("-movflags").arg("frag_keyframe+empty_moov+faststart")
    ...
    pub fn generate_fmp4_init_segment(&mut self) -> Result<Vec<u8>> {
        ...
        let ftyp = [
            ...
            b'i', b's', b'o', b'2',
            b'a', b'v', b'c', b'1', // avc1 for H.264
    ...
    pub fn packet_to_fmp4_segment(&self, sample: &FFmpegSample) -> Result<Vec<u8>> {
        debug!("Converting packet to fMP4 segment: {} bytes", sample.data.len());
