...
pub use super::mp4_parser::{BoxContent, MseSegment, Mp4Track};
...
impl FragmentWriter {
    ...
    pub fn generate_mse_segments(&mut self, boxes: &[BoxInfo], is_fragmented: bool) -> Result<Vec<MseSegment>, io::Error> {
        if is_fragmented {
    ...
    fn extract_fragmented_segments(&self, boxes: &[BoxInfo]) -> Result<Vec<MseSegment>, io::Error> {
            ...
            match box_info.header.box_type.as_str() {
                "ftyp" | "moov" => {
                    ...
                    if box_data.len() >= 8 {
                        ...
                        debug!("Added {} box to initialization segment ({} bytes)", 
                               box_info.header.box_type, box_data.len());
                    }
                }
                _ => {}
            }
        ...
        let mut i = 0;
        while i < boxes.len() {
            if boxes[i].header.box_type == "moof" && i + 1 < boxes.len() && boxes[i + 1].header.box_type == "mdat" {
    ...
    fn create_fragments_from_regular_mp4(&self, boxes: &[BoxInfo]) -> Result<Vec<MseSegment>, io::Error> {
