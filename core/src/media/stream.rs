...
impl MediaStreamImpl {
    ...
    pub async fn read(&mut self, size: usize) -> Result<Vec<u8>, MediaError> {
        ...
        if buffer.is_empty() {
            ...
            buffer.push(&data).map_err(|e| MediaError::from(e.to_string()))?;
        }
        
        // Get data from the buffer
        let data = buffer.pop(size).map_err(|e| MediaError::from(e.to_string()))?;
        ...
        if let Some(codec_name) = &self.current_codec {
            ...
            codec.decode(&data).await.map_err(|e| MediaError::from(e.to_string()))
        } else {
            Ok(data)
        }
    }
    ...
    pub async fn write(&mut self, data: &[u8]) -> Result<(), MediaError> {
        let encoded_data = if let Some(codec_name) = &self.current_codec {
            ...
            codec.encode(data).await.map_err(|e| MediaError::from(e.to_string()))?
        } else {
            data.to_vec()
        };
        
        let mut buffer = self.buffer.lock().await;
        buffer.push(&encoded_data).map_err(|e| MediaError::from(e.to_string()))
    }
