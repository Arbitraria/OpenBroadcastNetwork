...
impl PubSubMetrics {
    ...
    pub fn record_message_received(&self, topic_id: &TopicId, peer_id: Option<&PeerId>, size: usize) {
...
impl TimeSeriesData {
    ...
    fn rotate_if_needed(&mut self) {
        ...
        if elapsed >= self.bucket_size {
            let buckets_to_rotate = std::cmp::min(
                (elapsed / self.bucket_size) as usize,
                self.num_buckets,
            );
