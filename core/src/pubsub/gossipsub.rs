...
impl GossipSubService {
    ...
    fn to_gossipsub_message(&self, message: &Message) -> Result<(Vec<u8>, Option<LibP2PMessageId>), PubSubError> {
        // Serialize the message
        let payload = match serde_json::to_vec(message) {
            Ok(data) => data,
            Err(e) => return Err(PubSubError::PublishError(format!("Failed to serialize message: {}", e))),
        };
...
impl PubSub for GossipSubService {
    fn subscribe(&mut self, topic: &Topic) -> Result<(), PubSubError> {
        ...
        let mut topics = self.topics.write().map_err(|_| {
            PubSubError::SubscriptionError("Failed to acquire lock".to_string())
        })?;
        ...
        if topics.len() >= self.config.max_subscriptions {
            return Err(PubSubError::SubscriptionError(
                format!("Maximum subscriptions ({}) reached", self.config.max_subscriptions)
            ));
        }
    ...
    fn start(&mut self) -> Result<(), PubSubError> {
        ...
        // Store sender
        let mut event_sender = self.event_sender.lock().map_err(|_| {
            PubSubError::NetworkError("Failed to acquire lock".to_string())
        })?;
