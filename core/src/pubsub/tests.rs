...
use crate::pubsub::interface::{AsyncPubSub, PubSub};
use crate::pubsub::message::{Message, MessagePayload, MessageType};
use crate::pubsub::topic::{StreamMetadata, StreamTopic, Topic, TopicConfig, TopicId};
...
fn test_topic_creation() {
    ...
    let topic_with_desc =
        Topic::new("test-topic-2", "Test Topic 2").with_description("A test topic");
...
fn test_stream_topic_creation() {
    ...
    let stream_topic_with_meta =
        StreamTopic::new("stream-456", "Cool Stream").with_metadata(metadata);

    assert_eq!(
        stream_topic_with_meta.metadata.title.unwrap(),
        "Cool Stream"
    );
    assert_eq!(
        stream_topic_with_meta.metadata.content_type.unwrap(),
        "video/webm"
    );
    assert_eq!(stream_topic_with_meta.metadata.creator.unwrap(), "user-456");
    assert_eq!(stream_topic_with_meta.metadata.tags.len(), 2);
    assert!(stream_topic_with_meta
        .metadata
        .tags
...
fn test_basic_validator() {
    let topic_id = TopicId("test-topic".to_string());
    let validator = BasicValidator::new().with_max_message_size(100);

    // Valid message (small)
    let valid_msg = Message::binary(topic_id.clone(), vec![0; 50]);
    assert_eq!(
        validator.validate(&valid_msg, None),
    ...
    assert_eq!(
        validator.validate(&oversized_msg, None),
...
async fn test_gossipsub_service() {
    ...
    // Test unsubscribing
    assert!(AsyncPubSub::unsubscribe(&mut service, &topic.id)
        .await
        .is_ok());
...
#[tokio::test]
#[ignore = "Requires real swarm event loop — loopback publish does not trigger event_stream"]
