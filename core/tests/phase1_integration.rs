...
async fn test_bootstrap_peer_discovery() {
    ...
    // Start discovery
    discovery1.start().await.expect("Failed to start discovery1");
    discovery2.start().await.expect("Failed to start discovery2");
    ...
    assert!(!peers1.is_empty() || !peers2.is_empty(), "No peers discovered");
}
...
async fn test_overlay_network_basic() {
    ...
    let overlay = Libp2pOverlay::new(config).await.expect("Failed to create overlay");
...
async fn test_stream_management() {
    ...
    assert!(streams.contains(&stream_id), "Stream not found in active streams");
    
    // Stop the stream
    overlay.stop_stream(&stream_id).await
        .expect("Failed to stop stream");
    
    // Verify stream is removed
    let streams_after = overlay.active_streams().await
        .expect("Failed to get active streams after stop");
    assert!(!streams_after.contains(&stream_id), "Stream still active after stop");
...
async fn test_two_node_communication() {
    ...
    let node1 = Arc::new(Libp2pOverlay::new(config1).await.expect("Failed to create node1"));
    let node2 = Arc::new(Libp2pOverlay::new(config2).await.expect("Failed to create node2"));
    ...
    // Clean up
    node1.stop_stream(&stream_id).await.expect("Failed to stop stream");
...
async fn test_topology_rebalancing() {
    ...
    overlay.rebalance_topology().await
...
async fn test_phase1_complete_integration() {
    ...
    for i in 0..3 {
        ...
        let node = Arc::new(Libp2pOverlay::new(config).await.expect(&format!("Failed to create node {}", i)));
        let node_id = node.local_peer_id();
        
        node.start().await.expect(&format!("Failed to start node {}", i));
    ...
    nodes[0].publish_stream(&stream_id).await
        .expect("Failed to publish stream");
    
    // Relay the stream to other nodes
    for i in 1..nodes.len() {
        nodes[0].relay_stream(&stream_id, &node_ids[i]).await
    ...
    nodes[0].stop_stream(&stream_id).await
        .expect("Failed to stop stream");
...
async fn test_error_handling() {
    ...
    overlay.start().await.expect("Failed to start overlay with invalid peers");
