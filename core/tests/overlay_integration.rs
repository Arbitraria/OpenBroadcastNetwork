...
use OpenBroadcastNetwork_core::overlay::interface::{OverlayConfig, OverlayEvent, Overlay};
...
async fn test_overlay_basic_functionality() {
    ...
    assert!(overlay_result.is_ok(), "Failed to create overlay: {:?}", overlay_result.err());
    ...
    assert!(start_result.is_ok(), "Failed to start overlay: {:?}", start_result.err());
    assert!(overlay.is_running());
    
    // Test stats retrieval
    let stats_result = overlay.stats().await;
    assert!(stats_result.is_ok(), "Failed to get stats: {:?}", stats_result.err());
    
    // Test stop
    let stop_result = overlay.stop().await;
    assert!(stop_result.is_ok(), "Failed to stop overlay: {:?}", stop_result.err());
    assert!(!overlay.is_running());
}
