...
use OpenBroadcastNetwork_core::discovery::{
    ...
    DiscoveryEvent, DiscoveryManager, DiscoveryManagerConfig, PeerInfo,
};
...
async fn test_bootstrap_discovery_basic() {
    ...
    discovery
        .start()
        .await
        .expect("Failed to start bootstrap discovery");
    assert!(discovery.is_running());
    ...
    discovery
        .stop()
        .await
        .expect("Failed to stop bootstrap discovery");
    assert!(!discovery.is_running());
...
async fn test_dht_discovery_basic() {
    ...
    discovery
        .stop()
        .await
        .expect("Failed to stop DHT discovery");
    assert!(!discovery.is_running());
...
async fn test_discovery_manager_basic() {
    ...
    manager
        .start()
        .await
        .expect("Failed to start discovery manager");
    assert!(manager.is_running().await);
    ...
    manager
        .stop()
        .await
        .expect("Failed to stop discovery manager");
    assert!(!manager.is_running().await);
...
async fn test_peer_announcement() {
    ...
    let peer_info = PeerInfo {
        ...
        connection_status:
            OpenBroadcastNetwork_core::discovery::interface::ConnectionStatus::Connected,
    };

    // Announce the peer
    discovery
        .announce(peer_info.clone())
        .await
        .expect("Failed to announce peer");

    // Try to look up the peer
    let found_peer = discovery
        .lookup_peer(&peer_info.id)
        .await
...
async fn test_discovery_without_network() {
    ...
    let peers = dht
        .find_peers("")
        .await
        .expect("Should be able to search for peers");
    assert!(
        peers.is_empty(),
        "Should not find any peers without network"
    );
